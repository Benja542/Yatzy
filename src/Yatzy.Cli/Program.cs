...
public static class Program
{
    public static int Main(string[] args)
    {
        ...
        {
            case "tabel":
                PrintProbabilityTable(GetLong(args, "--kast", 200_000));
                return 0;
            case "analytisk":
                PrintAnalytic();
                return 0;
    ...
    private static void PrintHelp()
    {
        Console.WriteLine("""
            ...
            Brug:
              ...
              yatzy analytisk            De analytiske formler og deres led, skrevet ud
    ...
    private static void PrintProbabilityTable(long trials)
    {
        Console.WriteLine($"Sandsynlighed for at slå hvert slag. Monte Carlo: {trials:N0} simulerede ture pr. slag.");
        Console.WriteLine();
        Console.WriteLine(
            "{0,-16} {1,12} {2,12} {3,12} {4,10} {5,12}",
            "Slag", "Analytisk*", "Udfaldstræ", "Monte Carlo", "± 95 %", "Afvigelse");
        Console.WriteLine(new string('-', 80));

        var rows = ProbabilityReport.Build(Categories.All, counts: null, rerollsLeft: YatzyRules.RollsPerTurn, trials, seed: 12345);
    ...
    private static void PrintAnalytic()
    {
        foreach (var result in AnalyticProbability.All())
        {
            var bruteForce = AnalyticProbability.BruteForceCount(result.Category);
